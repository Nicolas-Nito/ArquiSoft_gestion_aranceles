...
def Consumer(service,callback):
    ...
    channel.exchange_declare(exchange='aranceles',exchange_type=ExchangeType.topic)
    ...
    channel.queue_bind(exchange='aranceles', queue=queue.method.queue, routing_key=f'{service}.*.*')
...
def publish_event(event: str, body: dict):
    ...
    channel.basic_publish(
        exchange='aranceles',  # O puedes usar un exchange personalizado
